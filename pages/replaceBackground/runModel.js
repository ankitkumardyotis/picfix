...
function ReplaceBackgroundModel() {
    ...
    // async function generateReplaceBackground(urlFromColorization) {
    ...
    async function generateReplaceBackground(fileUrl) {
        await new Promise((resolve) => setTimeout(resolve, 500));
        setLoading(true);
        const res = await fetch("/api/generateReplaceBackground", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
        ...
        if (res.status !== 200) {
            ...
            setRestoredPhoto(newPhoto.image);
            setError(null)
            setLoadCircularProgress(false)
    ...
    const handleClicktoGo = () => {
        ...
        generateReplaceBackground(fileUrl);
        if (inputValue.trim() === '') {
            setHighlight(true);
        } else {
            if (inputValue.trim() === '') {
    ...
    console.log("restoredPhoto",restoredPhoto)
...
export default ReplaceBackgroundModel;
