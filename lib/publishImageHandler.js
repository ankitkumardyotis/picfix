...
export async function handleImagePublish(publishData) {
    ...
    try {
        console.log('Starting image publish process...', { model, userId, title });
        ...
        console.log('Output image uploaded:', outputResult.path);
        ...
        for (let i = 0; i < inputImages.length; i++) {
            ...
            if (inputImage.url) {
                try {
                    ...
                    console.log(`Input image ${i + 1} uploaded:`, inputResult.path);
                } catch (error) {
        ...
        console.log('Published image saved to database:', publishedImage.id);
    ...
    switch (selectedModel) {
        ...
        case 'cartoonify':
            if (currentState.cartoonifyImage) {
                inputImages.push({
                    url: currentState.cartoonifyImage,
                    type: 'original'
                });
            }
            break;
