...
export async function storeGeneratedImage({
  ...
  try {
    console.log('Storing generated image to user history:', { userId, model, prompt: prompt?.substring(0, 50) });
    ...
    console.log('Image uploaded to R2 history path:', imagePath);
...
export async function publishFromHistory(historyId, publishData) {
  try {
    ...
    console.log('Published reference created (no upload):', publishedImage.id);
    console.log('Image accessible at existing path:', historyRecord.outputImagePath);
...
export function getModelInputImages(selectedModel, currentState) {
  ...
  switch (selectedModel) {
    ...
    case 'cartoonify':
      if (currentState.cartoonifyImage) {
        inputImages.push(currentState.cartoonifyImage);
      }
      break;
