...
export async function applyWatermark(imageBuffer, outputFormat = 'jpeg') {
  try {
    ...
    const watermarkedBuffer = await image
      .composite([{
        input: watermark,
        top: Math.floor(metadata.height - watermarkMetadata.height - watermarkMetadata.height/2), // Position at bottom
        left: Math.floor(metadata.width - watermarkMetadata.width - watermarkMetadata.width/2), // Position at right
        blend: 'over',
      }])
      .jpeg({ quality: 100 }) 
      .toBuffer();
